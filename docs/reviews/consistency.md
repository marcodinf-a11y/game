...
### ~~L5. Overlapping Policy Changes Underspecified~~ ✅ Resolved
...
**Resolution:** Adopted the **replace** model — the latest value overwrites any pending change for the same parameter, and the lag timer resets. This is the standard approach in economic/political simulation games:
...
- **Victoria 3:** Structural changes (law enactments) allow only one in-progress change per category. Changing direction means canceling and restarting — effectively replace with an explicit cancel step. (Victoria 3's model suits structural reforms with success/failure mechanics, not numerical adjustments.)
- **EU4 / HOI4:** Numerical adjustments (tax sliders, production) are immediate or near-immediate. Structural changes use cooldown timers that prevent re-adjustment for N months — closer to "reject," but framed as a timer rather than a hard block.

This game's three policy levers (tax rate, spending level, spending allocation) are numerical/slider-style controls, making the Democracy model the closest fit. Replace is simplest to implement (the `IPolicyPipeline` entry updates its target value), matches the player mental model, avoids surprising queue buildup, and fully respects FR-CTL-001 (the player can always adjust). The lag still applies — the player gets instant *correction* of intent, not instant *effect*.
...
### ~~L12. No Error Recovery Strategy for SFC Failures~~ ✅ Resolved
...
**Resolution:** Adopted a log-and-ask strategy. An SFC failure cannot be caused through the game's typed interface — every transaction is double-entry, generated by the engine's own code. The only two causes are (1) an undiscovered bug in the simulation, or (2) external memory manipulation (e.g., Cheat Engine). The recovery flow:

1. `CheckConsistency()` fails → log full diagnostic details (which accounts, what imbalance, which tick, expected vs. actual totals).
2. Pause the simulation and show a dialog to the player: *"Accounting inconsistency detected at tick N. This is either a bug (please report it with the log file) or caused by external memory modification. You can continue with the inconsistent state, or restore to the last consistent state."*
3. **Continue** → unpause and keep ticking with the broken state. This covers the Cheat Engine case — if the player deliberately modified memory, let them have it.
4. **Restore** → replay the simulation from tick 0 to tick N-1 using the policy input log and the original seed. The simulation is deterministic (seeded `IRandom`), so replaying with the same inputs reproduces the exact state at any prior tick. This reconstructs the last known-good state without needing snapshots or transaction rollback.

The replay-based restore reuses the same deterministic replay infrastructure planned for post-MVP save/load (see PRD Section 5, save/load out-of-scope note). The policy input log — a list of `(tick, command)` pairs — must be recorded by the Game Controller during play. Replaying without rendering is fast, so even long games restore in acceptable time.

Updated NFR-DTA-001 in the PRD to specify log-and-ask behavior. Updated Architecture Section 4.1 Accounting Phase to describe the failure flow (log, pause, player choice). Added `PolicyInputLog` recording responsibility to Architecture Section 2.2 (Game Controller). Added `ReplayToTick(int targetTick)` method to `ISimulationFactory` in Architecture Section 3.7, taking the original seed and a policy input log.
